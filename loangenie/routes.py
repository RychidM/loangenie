...
from loangenie import app, db, configure, bcrypt
...
def dashboard():
    ...
    return render_template('dashboard.html', name=current_user.username, image_file =image_file, dateCrr = datetime.utcnow)
...
def userAccount():
    image_file = url_for('static', filename='img/' + current_user.image_file)
    return render_template('userAccount.html', image_file =image_file, name= current_user.name, org = current_user.organization, emailAdd=current_user.email, passWord=current_user.password)
